...
def _write_measurement(channel):
    global options
...
def _setup_gpio(channel):
    GPIO.setmode(GPIO.BCM)
    GPIO.setup(channel, GPIO.IN, pull_up_down=GPIO.PUD_DOWN)
    GPIO.add_event_detect(channel, GPIO.RISING, callback=_write_measurement, bouncetime=1000)


def _read_parser():
    global options
    parser = argparse.ArgumentParser(conflict_handler="resolve",
                                     description="This will track the IR Sensor on specific channel and post events in InfluxDB")
